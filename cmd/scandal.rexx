...
objectif   : trouver, trier et archiver tout ou partie du(des) journal(ux)
             g�n�r�(s) pendant la sauvegarde d'un h�te d�fini.
...
/* d�claration et construction de variables *********************************************************************/
serveur=hostname
...
if clientfile \= "" then do
   ...
   /***************************************************************************************************************
   la partie ci-dessous est a am�liorer car, POUR SIGTSM, elle ne fonctionne que si la sauvegarde se finie
   avant minuit... + V�rifier aussi que dans la recherche, la date apparaisse en d�but de ligne et pas en fin...
   ***************************************************************************************************************/
...
objectif   : fournir un bilan rapide du journal de sauvegarde � analyser.
        + sauvegarde r�ussie = oui/non
        + dur�e de la sauvegarde
        + volumetrie sauvegard�e
...
Select
   When Pos("host=", msgCmd) = 0 Then Do
      Tm = DISPLAY("ERREUR SYNTAXE : commande doit �tre :")
...
If Pos(",date", msgCmd) \= 0 then Do
   ...
   If length(cejour) >= 4 then Do
      ...
      Select
         ...
         End
         OtherWise Do
         Tm = DISPLAY("ERREUR SYNTAXE : date="w_date" erron�e. Doit �tre : date=jjmm[aaaa]")
         return
         End
      End
      jour_S = aaaa""mm""jj
...
/* d�claration et construction de variables & Initialisation de variables *****************************************/
serveur    = hostname
...
info.serveur.2    = ""                                            /* on y stockera info sur dur�e de sauvegarde   */
info.serveur.3    = ""                                            /* on y stockera info sur volume sauvegard�     */
   ...
   If cefich.0 \= 0 Then Do
      Select
         ...
         /* RECHERCHE 11   : LA SAUVEGARDE A-T'ELLE REUSSIE ?
            ...
            position111    : n'importe o�
            RESULTATs      : si r�ussie   : test111 = 1
                           : si �chou�e   : test111 = 0
         */
            ...
            If test111.1 \= 0 then Do
               test.serveur.111 = 1
               msg.serveur.111 = "OK : Pr�sence phrase 'Scheduled event '...' completed successfully'."
            End
            Else Do
               test.serveur.111 = 0
               msg.serveur.111 = "KO : Pr�sence phrase 'Scheduled event '...' completed successfully'."
            End
...
/*DEBUG-  say "test."serveur".111 = "test.serveur.111    */
         ...
         /* test112        : trouver ligne avec "Waiting to be contacted by the server."
            etape1121      : d�finir sa position si elle existe.
                           : si existe pas   : valeur = 0
                           : si existe   : poursuivre
            etape1122      : trouver le nombre de ligne du fichier.
            etape1123      : comparer � la position d�finie � l'etape1_1_2_1
...
/*DEBUG-        say "etape."serveur".1122 = "etape.serveur.1122   */
                  If etape.serveur.1122 = etape.serveur.1121 then Do
                     test.serveur.112 = 2   /* phrase existe en derni�re ligne       */
                     msg.serveur.112 = "OK : Pr�sence phrase 'Waiting to be contacted by the server'."
                  End
                  Else Do
                     test.serveur.112 = 1
                     msg.serveur.112 = "PB : Pr�sence phrase 'Waiting to be contacted by the server' mais pas en derni�re ligne."
                  End
               End
            End
            Else Do
               test.serveur.112 = 0
               msg.serveur.112 = "KO : Pr�sence phrase 'Waiting to be contacted by the server'."
            End
...
/*DEBUG-  say "test."serveur".112 = "test.serveur.112    */
         ...
         /* test113        : v�rifier si tous volumes qui devaient �tre sauvegard�s l'ont �t�,
                           : et d�finir (comme �a en passant) la dur�e de sauvegarde pour chaque
                           : volume.
            etape1131      : obtenir la liste des volumes � sauvegarder par
                           : grep "Incremental backup of volume" [fichier] | awk '{print $7}'
                           : compter nombre de volume � sauvegarder = etape1131.0
            etape1132      : pour chaque '/--' si on a une ligne "Successful incremental backup of '/--'" ?
                           ...
                           : x variant de 1 � etape1131.0
            etape1133      : compter nombre de volume sauvegard�s
            RESULTATs      : si etape1132 = etape1131.0   : test113 = 1 & msg113 = ""
                           : si etape1132 \= etape1131.0   : test113 = 0 & msg113 = rapport volumes non sauvegard�s / volumes a sauvegarder
         */
...
/*DEBUG-  say "etape1131.0 = "etape1131.0       */
            If etape1131.0 \= 0 then Do
               ...
               If etape.serveur.1133 = etape.serveur.1131 then Do
                  test.serveur.113  = 1
                  msg.serveur.113 = "OK : "etape.serveur.1133" volumes sauvegard�s / "etape.serveur.1131
               End
               Else Do
                     test.serveur.113 = 0
                     msg.serveur.113 = "KO : "etape.serveur.1133" volumes sauvegard�s / "etape.serveur.1131
               End
...
/*DEBUG-     say "test."serveur".113 = "test.serveur.113       */
         ...
         /* RECHERCHE 12   : DUREE DE LA SAUVEGARDE
            test121        : (dur�e sauvegarde compl�te) trouver ligne
                           : mm/dd/yy"   "heure" "Elapsed processing time:           "hh:mm:ss
            position121    : n'importe o�
            RESULTATs      : test121 = hh:mm:ss    = dur�e de la sauvegarde
         */
               ...
               If test121.0 \= "" then Do
                  ...
                  If stest.serveur.121.MAXI >= stest.serveur.121 then Do
                     msg.serveur.121 = "OK : Dur�e de la sauvegarde = "test.serveur.121" < � fen�tre de 6h00."
                  End
                  Else Do
                     msg.serveur.121 = "PB : Dur�e de la sauvegarde = "test.serveur.121" > � fen�tre de 6h00."
                  End
               End
               Else Do
                  test.serveur.121 = "n/d"
                  msg.serveur.121 = "PB : Dur�e de la sauvegarde = non d�finissable."
               End
               If ViewdoScan = 1 then Do
...
/*DEBUG-     say "DUREE de sauvegarde : test."serveur".121 = "test.serveur.121       */
         ...
         /* test122        : (dur�e sauvegarde de chaque volume)
            etape1221      : trouver heure d�but de sauvegarde du premier volume, avec la commande
            ...
            s/etape1223    : faire la diff�rence avec les heures de fin
...
/*DEBUG-        say "Heure de fin de sauvegarde de volume "etape1131.x     */
                  ...
                  Else Do
                     ...
                     Ajout122 = "KO = "
                  End
                  msg.serveur.122.x = Ajout122"Dur�e de sauvegarde du volume "etape.serveur.1131.x" = "etape.serveur.1223.x
                  If ViewdoScan = 1 then Do
         ...
         /* RECHERCHE 13      : volumetrie sauvegard�e
               test131        : trouver ligne mm/dd/yy"   "heure" "Total number of bytes transferred:    "volumetrie
               position131    : n'importe o�
               RESULTATs      : test131 = volumetrie (ex:146,5 GB) = volumetrie sauvegard�e
         */
...
/*DEBUG-     say "test."serveur".131 = "test.serveur.131          */
            ...
            Else Do
               /*
               On prend ici, en compte le cas o� la commande :
               "grep 'Incremental backup of volume' "cefich.1" | awk '{print $7}'"
               ne renvoie aucunes correspondances. On d�fini d'une autre fa�on les variables :
                  test113
                  test121
                  test131
               */
               /*
                  RECHERCHE 11   : LA SAUVEGARDE A-T'ELLE REUSSIE :
                     test111     : d�fini plus haut
                     test112     : d�fini plus haut
                     test113     : ZERO volumes � sauvegarder."
...
/*DEBUG-     say "test."serveur".113 inutile ici donc = "test.serveur.113      */
               ...
               /* DEBUT MODIFICATION "M1a" : ESAU - DATE : 17.09.2004
                  MOTIF : bug si pr�sence de plusieurs s�quences infructueuses :
                        "Querying server for next scheduled event."
                        .... (pas d'information de dur�e de sauvegarde) ...
                        "Waiting to be contacted by the server."
                        avant la pr�sence d'une s�quence contenant les informations de dur�e de sauvegarde.
                  ISSUE : si on ne peut pas d�terminer de dur�e de sauvegarde
                        m1in  : fichier d'entr�e = cefich.1
                        ...
                        tant que m1_1 > 0, alors
                           ...
                           m1a2  : on en d�duit le nouveau nb de ligne fichier (� cr�er) = m1a2 - m1a1
                           m1a3  : on copie les m1_2 derni�res lignes du fichier [m1ain] dans un fichier temporaire [m1aout]
                                 : on recherche la dur�e de sauvegarde : cf. test121
                           si dur�e non trouv�e, on boucle
                           si dur�e d�termin�e, on quite  (m1a1 = 0)
               */
               m1a1 = 1
               m1acount = 1
               m1ain = cefich.1
               Do while m1a1 > 0
                  ...
                  If m1a1.0 > 0 then Do
                     ...
                     /*
                     RECHERCHE 12   : DUREE DE LA SAUVEGARDE
                        test121     : faire la diff�rence entre heure d�but et heure fin
...
/*DEBUG-     say "etape."serveur".121 = "etape.serveur.121           */
                     If etape.serveur.121 = 2 then Do
                        ...
                        If stest.serveur.121.MAXI >= stest.serveur.121 Then Do
                           msg.serveur.121 = "OK : Dur�e de sauvegarde = "test.serveur.121" < � fen�tre de 2h00."
                           m1a1 = 0
                        End
                        Else Do
                           msg.serveur.121 = "PB : Dur�e de sauvegarde = "test.serveur.121" > � fen�tre de 2h00."
                           m1a1 = 0
                        End
/*DEBUG-        say "   : Dur�e de sauvegarde : test."serveur".121 = "test.serveur.121          */
                     End
                     If etape.serveur.121 < 2 Then Do
                        test.serveur.121 = "n/d"
                        msg.serveur.121 = "PB : Dur�e de sauvegarde = non d�finissable."
                        m1a1 = 0
                     End
                     If etape.serveur.121 > 2 then Do
                     m1a1 = 1
               ...
               /*
                  RECHERCHE 13   : VOLUMETRIE SAUVEGARDEE
                     test131     : pas d�finissable   : test131 = -1
               */
               test.serveur.131 = -1
/*DEBUG-     say "test131 ind�finissable ici donc = "test.serveur.131      */
            End
         End
         ...
         /* RECHERCHE 21   : LA SAUVEGARDE A-T'ELLE REUSSIE
               ...
               position211 : derni�re ligne
         */
               Address System "grep 'Sauvegarde completee avec succes' "cefich.1 WITH OUTPUT STEM test211.
               If test211.0 \= 0 then Do
                  test.serveur.211 = 1
                  msg.serveur.211 = "OK : Pr�sence phrase 'Sauvegarde completee avec succes'."
               End
               Else Do
                  test.serveur.211 = 0
                  msg.serveur.211 = "KO : Pr�sence phrase 'Sauvegarde completee avec succes'."
               End
...
/*DEBUG-     say "test."serveur".211 = "test.serveur.211          */
         ...
         */
               ...
               If test212.1 = 0 then Do
                  test.serveur.212 = 1
                  msg.serveur.212 = "OK : "test212.1" message(s) 'error' trouv�(s)."
               End
               Else Do
                  test.serveur.212 = 0
                  msg.serveur.212 = "KO : "test212.1" message(s) 'error' trouv�(s)."
               End
...
Select
   When i=2 then Do
      If length(option.2) >= 4 then Do
         ...
         Select
            ...
            End
            OtherWise Do
            Tm = DISPLAY("ERREUR SYNTAXE : date="option.2" erron�e. Doit �tre : date=jjmm[aaaa]")
            return
            End
         End
         jour_S = jj""mm""aaaa
   ...
   End
   OtherWise Do
      Tm = DISPLAY("ERREUR SYNTAXE : "msgCmd" erron�e. Doit �tre : stat hostname [jjmm[aaaa]]")
      return
   End
End
